find.nodes <- function(patterns,env) function(x) {
...
make.image.raster <- function(image.default) {
  ...
  invisible(rapply(fnbody,find.nodes(patterns,here),how="list"))
...
make.filled.contour.nolegend <- function(filled.contour) {
